...
// используем рекурсию для проверки направления хода // поле, направление хода, тип шашки, координаты шашки (ряд/кол), прыжок (если был)
static void checkDirection(Square grid[], const Direction& direction, const CheckerType checker, const int row, const int col, const bool jumped, const bool eflag, const int KingDirection)
{
 ...
 if (!isOutOfBounds(nextRow, nextCol))
 {
  ...
  if (areOppositeCheckers(checker, squareAtDirection->getChecker()))
  {
   ...
   if (!isOutOfBounds(nextRow2, nextCol2))
   {
    ...
    if (nextSquareAtDirection->getChecker() == CheckerType::NONE && nextSquareAtDirection->getHighlight() != HighlightType::NEXT_POSSIBLE_POSITION)
    {
     ...
     // сохраняем возможную для хода позицию
     ...
     if (checker == CheckerType::RED)
     {
      ...
      for (int i = 0; i < 4; ++i) checkDirection(grid, directions[i], checker, nextRow2, nextCol2, true, true, KingDirection); // (int i = 0; i < 2; ++i)
     }
     // если черная фишка
     else if (checker == CheckerType::BLACK)
     {
      // проверяем south west и south east
      for (int i = 0; i < 4; ++i) checkDirection(grid, directions[i], checker, nextRow2, nextCol2, true, true, KingDirection); // (int i = 2; i < 4; ++i)
     }
     // далее проверяем дамок
     else
     {
      // проверяем все направления
      for (int i = 0; i < 4; ++i) checkDirection(grid, directions[i], checker, nextRow2, nextCol2, true, true, KingDirection); // (int i = 0; i < 4; ++i)
     }
    }
  ...
  // если соседний квадрат пуст и мы никого не ели (ефлаг нужен для пешек, чтоб заразы не могли походить назад в первом поколении)
  else if (squareAtDirection->getChecker() == CheckerType::NONE && !jumped && eflag)
  {

   if (checker == CheckerType::KING_RED || checker == CheckerType::KING_BLACK)
   {
     // если следующий по направлению квадрат не выделен как возможный для хода
     if (squareAtDirection->getHighlight() != HighlightType::NEXT_POSSIBLE_POSITION)
     {
      // выделяем следующий возможный ход
      squareAtDirection->setHighlightTo(HighlightType::NEXT_POSSIBLE_POSITION);

      // сохраняем возможную для хода позицию
      squareAtDirection->setPreviousSquare(&grid[getIndex(row, col)]);

      // создаем возможные направления хода
      Direction directions2[4] = { northWest, northEast, southWest, southEast };

      // рекурсивно проверяем все направления
      if (KingDirection == 0)
       checkDirection(grid, directions2[0], checker, nextRow, nextCol, false, true, KingDirection);
      else if (KingDirection == 1)
       checkDirection(grid, directions2[1], checker, nextRow, nextCol, false, true, KingDirection);
      else if (KingDirection == 2)
       checkDirection(grid, directions2[2], checker, nextRow, nextCol, false, true, KingDirection);
      else if (KingDirection == 3)
       checkDirection(grid, directions2[3], checker, nextRow, nextCol, false, true, KingDirection);
     }
   }
   else
   {
...
int main(int argc, char* argv[])
{
 ...
 while (!stop)
 {
  ...
  if (gameState == GameState::SELECTED)
  {
   ...
   // костыльная переменная для определения направления хода дамки
   int KingDirection = 0;

   // если белая фишка
   if (checker == CheckerType::RED)
   {
    // проверяем north west и north east, а потом всё остальное
    for (int i = 0; i < 4; ++i) 
     if (i < 2)
     checkDirection(checkerboard, directions[i], checker, row, col, false, true, KingDirection); // (int i = 0; i < 2; ++i)
     else checkDirection(checkerboard, directions[i], checker, row, col, false, false, KingDirection);
   }
   // если черная фишка
   else if (checker == CheckerType::BLACK)
   {
    // проверяем south west и south east, а потом всё остальное
    for (int i = 0; i < 4; ++i) 
     if (i >= 2 && i < 4)
     checkDirection(checkerboard, directions[i], checker, row, col, false, true, KingDirection); // (int i = 2; i < 4; ++i)
     else checkDirection(checkerboard, directions[i], checker, row, col, false, false, KingDirection);
   }
   // далее проверяем дамок
   else
   {
    ...
    {
     KingDirection = i;
     checkDirection(checkerboard, directions[i], checker, row, col, false, true, KingDirection); // (int i = 0; i < 4; ++i)
    }
   }
 ...
 if (winner == Winner::RED) std::cout << "The winner is WHITE!" << std::endl;
  ...
  if (areOppositeCheckers(checker, squareAtDirection->getChecker()))
  {
   ...
   if (!isOutOfBounds(nextRow2, nextCol2))
   {
    ...
    if (nextSquareAtDirection->getChecker() == CheckerType::NONE && nextSquareAtDirection->getHighlight() != HighlightType::NEXT_POSSIBLE_POSITION)
    {
     ...
     {
      // проверяем north west и north east
      for (int i = 0; i < 2; ++i) checkDirection(grid, directions[i], checker, nextRow2, nextCol2, true); // (int i = 0; i < 2; ++i)
     }
     // если черная фишка
     else if (checker == CheckerType::BLACK)
     {
      // проверяем south west и south east
      for (int i = 2; i < 4; ++i) checkDirection(grid, directions[i], checker, nextRow2, nextCol2, true); // (int i = 2; i < 4; ++i)
     }
     // далее проверяем дамок
     else
     {
      // проверяем все направления
      for (int i = 0; i < 4; ++i) checkDirection(grid, directions[i], checker, nextRow2, nextCol2, true); // (int i = 0; i < 4; ++i)
     }
    }
